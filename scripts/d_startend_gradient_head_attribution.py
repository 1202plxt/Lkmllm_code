"""
d_startend_gradient_head_attribution.py — 仅针对 start/end 时间数字 token 的
梯度归因定位跨模态关键 Attention Head

本脚本是 c_gradient_head_attribution.py 的一个变体，与原版的三点核心区别：

  1. Query 位置收窄：不再用全部 answer token（"The event happens in 12.3 -
     45.6 seconds." 整句）做归因，而是只用其中 **start 时间数字** 和
     **end 时间数字** 对应的 token（可能各自被切成多个 sub-token，
     对这些 token 求均值）。grad_score 和 attn_align 两部分都统一改用
     这批位置，而不是像原版那样用全部 answer token。

     attn_align 额外做了一步归一化：不是直接算 GT 区间的原始注意力
     均值，而是先把 GT 区间拿到的注意力质量 sum 起来，再除以"这个
     query token 在因果 attention 下、完全均匀分配时该拿到的份额"
     （n_gt_tok / (p+1)，p 是该 token 在序列里的位置），得到"是随机
     水平的几倍"这个 ratio。原因：直接对原始注意力值做跨样本平均，
     会被不同样本的视频长度、GT 时长占比这些上下文差异污染（分母在
     样本间波动很大，数值不可比）；换成 ratio 后再跨样本平均，才是
     在同一把尺子上比较。

  2. 归因目标换成 target logit：不再对整句 answer 算 CE loss 再
     backward，而是取 teacher-forcing 下 start/end 数字 token 在真实
     GT 类别上的 logit 值（即模型预测这个 token 时，"标准答案"这个
     类别拿到的原始得分），把这些 logit 加总成一个标量，直接对这个
     标量做 backward。这跟 neuron_temporal.py 里 MLP 神经元归因的目标
     定义（score(i) = h_i × ∂(target_logit)/∂h_i）保持一致，而不是用
     CE loss 当目标。

  3. 显存策略——36 层分批 retain_grad：不再一次性给全部 36 层挂
     forward/backward hook 再只做一次 forward+backward（那样全部层的
     (heads, seq, seq) attention weights + 梯度会同时驻留显存，序列长
     跑起来很容易 OOM）。改成把 36 层按 --layers-per-batch（默认 6）
     切成若干批，每批只给这批层挂 hook，独立做一次完整的
     forward + backward，算完这批的分数立刻移除 hook、清空梯度、
     清理显存，再进入下一批。

     光是"只给这批层挂 hook"本身并不会省显存——只要 forward 时这批
     层之前和之后的所有层依然 requires_grad=True，PyTorch 就还是要把
     完整 36 层的 attention weights / 激活值都存起来等 backward 用，
     跟不分批一模一样。真正省显存的是 FreezeLayersNoGrad：每批开始
     前，把"这批层之前"的所有层的 forward 临时包进 torch.no_grad()，
     切断它们对反向图的贡献。注意这个优化对越靠后的 batch 效果越好——
     batch 之后到第 35 层，无论如何都得保留梯度（否则梯度传不到
     target logit），这部分省不掉；只有 batch 之前的层能冻结。所以
     覆盖靠前层号的 batch（比如 layer 0-5）后面还有 30 层要保留梯度，
     跟不分批时几乎一样费显存——这是反向传播的结构性限制，不是实现
     bug。如果连最前面的 batch 都 OOM，调小 --layers-per-batch 也没用，
     需要靠调低 --max-side / --fps 压缩 seq_len（attention weights
     显存跟 seq_len 是平方关系，收益最大）。单批 OOM 时脚本会捕获并
     跳过这一批，继续跑下一批，不会让整个样本失败。

用法：
    python Lkmllm_code/scripts/d_startend_gradient_head_attribution.py \
...
def build_inputs(processor, frames: list, query: str, duration: float,
                 ...
                 gt_start: float = 0.0, gt_end: float = 0.0) -> dict:
...
def load_samples(filtered_json: Path, video_dir: Path,
    ...
    """从 JSON 加载样本。兼容多种格式：queries 为 dict 列表或字符串列表。"""
    raw = json.loads(filtered_json.read_text(encoding="utf-8"))
    raw.pop("_meta", None)
    ...
    for s in sample_list:
        vp = video_dir / f"{s['video_id']}.mp4"
        if not vp.exists():
            for ext in [".mkv", ".avi", ".webm"]:
                if (video_dir / f"{s['video_id']}{ext}").exists():
                    break
            else:
                missing.append(s["video_id"])
    if missing:
        print(f"  [WARN] {len(missing)}/{len(sample_list)} 个视频缺失，跳过")
        sample_list = [s for s in sample_list if s["video_id"] not in set(missing)]
...
def main(argv=None):
    ...
    print(f"\n{'='*60}")
    print(f"开始 start/end token 梯度归因（{len(sample_list)} 样本，"
    ...
    for idx, s in enumerate(sample_list):
        try:
            # 1. 采帧
            frames = sample_frames(video_dir / f"{s['video_id']}.mp4", args.fps)
            ...
            inputs, text_input, answer_text = build_inputs(
                processor, frames, s["query"], s["duration"], device, args.fps,
            ...
            print(f"  [{idx+1}/{len(sample_list)}] {s['video_id']} "
            ...
            gt_tok_s, gt_tok_e = compute_gt_video_token_range(
                s["gt_start"], s["gt_end"], s["duration"], n_video,
            )
    ...
    print(f"\n{'='*60}")
    print(f"完成  有效样本={valid_count}  失败={failures}  耗时={elapsed:.1f}s"
          f"  ({elapsed/max(valid_count,1):.1f}s/sample)")

"""
m_e_head_eval.py — TimeLens Temporal Grounding 多卡评测

功能：
  1. 评测 m_heads_finetune_layer_lora_attn_align.py 保存并合并后的完整模型；
  2. 评测原始 Qwen3-VL-8B-Instruct base model；
  3. 每张 GPU 加载一份完整模型，按样本进行数据并行，最后汇总预测与指标；
  4. 输出 mIoU、R@1_IoU0.3、R@1_IoU0.5、R@1_IoU0.7 和逐样本结果。
...
说明：
  ...
  - --num-gpus 8：明确使用 8 张可见 GPU。
  - --num-gpus 1：单卡评测。
  - --max-samples 0：评测完整测试集；调试时可设为 500。
  - 微调结束后 LoRA 已合并到 --output-dir，直接把该目录传给
    --model-path，不需要单独加载 lora_layer_adapter.pt。
  - 当前脚本自行创建 GPU worker，不要使用 torchrun 启动。若外套
    torchrun --nproc_per_node 8，每个 rank 会再次创建 8 个 worker，造成
    重复评测和显存冲突。torchrun 只用于 m_ 微调脚本。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # 采样控制 - 默认对齐 TimeLens 官方评测设置
    p.add_argument("--max-samples", type=int, default=0,
                   help="默认 0 表示评完整测试集；调试时可手动设为 500 等小样本")
    p.add_argument("--sample-seed", type=int, default=42)
    p.add_argument("--fps",         type=float, default=2.0,
                   help="TimeLens 官方默认 FPS=2.0")
    p.add_argument("--max-frames",  type=int, default=0,
                   help="TimeLens 官方不额外设置 200 帧上限；0 表示不在脚本里截帧，只由 fps 和 total_tokens 控制")
    ...
    p.add_argument("--min-tokens",   type=int, default=64,
                   help="TimeLens 官方默认 min_tokens=64")
    p.add_argument("--total-tokens", type=int, default=14336,
                   help="TimeLens 官方默认 total_tokens=14336。"
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        duration = max(duration, float(s["gt_end"]))
        effective_fps = (len(frames) / duration) if duration > 0 else self.fps
        return {
            **{k: s[k] for k in ("video_id", "query", "gt_start", "gt_end")},
            "duration": duration,
            "frames": frames,
    ...
    for idx in pbar:
        ...
        try:
            (ps, pe), pred_raw = infer_generate(
                ...
                device, duration,
                sample_fps=item.get("fps", args.fps),
...
def main(argv=None):
    ...
    print(f"  total_tokens   = {args.total_tokens}   (TimeLens 官方默认 14336)")
    print(f"  attn_impl      = {args.attn_implementation}   (默认 flash_attention_2)")
